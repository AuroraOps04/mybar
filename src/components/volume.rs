...
impl<'a> Component for Volume<'a> {
    fn draw(&self) -> Result<(), MyBarError> {
        ...
        self.painter.draw_rounded_background(self.x as f64, iw + 10.0 * 2., 10.0, "#475164")?;
        self.painter.draw_text(self.x as f64 + 10.0, 10.0, &icon, color)?;
        
        let rw = self.width as f64 * v;
        self.painter.set_hex_color(&color)?;
        self.painter.cairo_conn.move_to(self.x as f64 + 10.0 + te + 5.0, 20.0);
        self.painter.cairo_conn.line_to(self.x as f64 + 10.0 + te + 5.0 + rw, 20.0);
    ...
    fn handle_event(&self, event: &Event) -> Result<(), MyBarError> {
        match event {
            Event::MouseClick { x, y, button } => {
                ...
                } else if button == 4 { // 滚轮上
                    let current_volume = self.audio.get_current_volume();
                    self.audio.set_current_volumn((current_volume + 0.05).min(1.0));
                    self.draw()?;
                } else if button == 5 { // 滚轮下
                    let current_volume = self.audio.get_current_volume();
                    self.audio.set_current_volumn((current_volume - 0.05).max(0.0));
                    self.draw()?;
                }
            }
